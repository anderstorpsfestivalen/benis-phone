...
import SIPInputNode from "./SIPInputNode";

const nodeTypes = {
  fnNode: FnNode,
  actionNode: ActionNode,
  queueNode: QueueNode,
  sipInputNode: SIPInputNode,
};
...
export default function FnGraph({
  ...
  queues: Queue[];
  sip: SIPConfig;
  onSelectSIP: (connectionID: string) => void;
  ...
  const { nodes, edges } = useMemo(() => {
    const g = buildNodesAndEdges(fns, queues, sip);
  ...
  const onNodeClick = useCallback<NodeMouseHandler>(
    (_, node) => {
      ...
      } else if (node.type === "sipInputNode") {
        const data = node.data as SIPInputNodeData;
        onSelectSIP(data.connectionID);
      } else {
        setSelection(null);
      }
    },
      ...
      >
        {!selection && (
          <div className="p-6 text-sm text-blue-slate">
            <p>
              Click a node to edit. SIP input nodes show every endpoint and
              trunk route into this shared flow.
            </p>
              ...
              <button
                onClick={() => {
                  if (selection.source.kind === "fn") {
                    ...
                    setSelection({
                      kind: "queue",
                      queueName: selection.source.queueName,
                    });
                  }
                }}
